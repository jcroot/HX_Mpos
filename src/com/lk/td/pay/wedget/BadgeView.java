...
public class BadgeView extends TextView {
    ...
    private static final int DEFAULT_BADGE_COLOR = Color.parseColor("#e85147"); //Color.RED;
...
//		RoundRectShape rr = new RoundRectShape(outerR, null, null);
    ...
    public void setMsgNumMoreStyle() {
        setBadgePosition(BadgeView.POSITION_TOP_RIGHT);// 显示的位置.右上角,BadgeView.POSITION_BOTTOM_LEFT,下左，还有其他几个属性
        setTextColor(Color.WHITE);// 文本颜色badgeView.setBadgeBackgroundColor(Color.RED);// 提醒信息的背景颜色
        if (getText().toString().length() < 3) {
            setTextSize(DensityUtil.sp2px(context, 5f));// 文本大小
        } else {
            setTextSize(DensityUtil.sp2px(context, 3.5f));// 文本大小
        }
//        setBadgeMargin(3, 3); // 水平和竖直方向的间距
        setWidth(DensityUtil.dp2px(context, 20));
        setHeight(DensityUtil.dp2px(context, 20));
        setGravity(Gravity.CENTER);
//        setBadgeMargin(5); //各边间隔
    }

    public void setMsgNumIndexStyle() {
        setBadgePosition(BadgeView.POSITION_CENTER);// 显示的位置.右上角,BadgeView.POSITION_BOTTOM_LEFT,下左，还有其他几个属性
        setTextColor(Color.WHITE);// 文本颜色badgeView.setBadgeBackgroundColor(Color.RED);// 提醒信息的背景颜色
        if (getText().toString().length() < 3) {
            setTextSize(DensityUtil.sp2px(context, 3.5f));// 文本大小
        } else {
            setTextSize(DensityUtil.sp2px(context, 3f));// 文本大小
        }
//        setBadgeMargin(3, 3); // 水平和竖直方向的间距
        setWidth(DensityUtil.dp2px(context, 20));
        setHeight(DensityUtil.dp2px(context, 20));
