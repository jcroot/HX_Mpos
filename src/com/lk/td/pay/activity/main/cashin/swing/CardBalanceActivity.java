...
public class CardBalanceActivity extends BaseActivity {
	...
	class MyEmvSwipeControllerListener implements EmvSwipeControllerListener {
		...
		public void promptForAmount() {
			...
			if (emvSwipeController.setAmount(amount, cashbackAmount, "840",
					transactionType)) {
		...
		public void onReturnCheckCardResult(CheckCardResult checkCardResult,
			...
			if (checkCardResult == CheckCardResult.NONE) {
				cashin_show_msg_text
						.setText(getString(R.string.no_card_detected));
				startSwingCard();
			} else if (checkCardResult == CheckCardResult.ICC) {
				PosData.getPosData().setMediaType("02");
				cashin_show_msg_text
						.setText(getString(R.string.icc_card_inserted));
		...
		public void onReturnCancelCheckCardResult(boolean isSuccess) {
			if (isSuccess) {
				cashin_show_msg_text
						.setText(R.string.cancel_check_card_success);
			} else {
		...
		public void onReturnTransactionResult(
			...
			if (transactionResult == TransactionResult.APPROVED) {
				...
				cashin_show_msg_text
						.setText(getString(R.string.transaction_cancel));
				...
				cashin_show_msg_text
						.setText(getString(R.string.card_not_supported));
				...
				cashin_show_msg_text
						.setText(getString(R.string.invalid_icc_data));
				...
				cashin_show_msg_text
						.setText(getString(R.string.icc_card_removed));
			}
		...
		public void onReturnBatchData(String tlv) {
			...
			content += "\nperiod:" + period;
			String icNumber = "000" + decodeData.get("5f34");
			// if(icNumber.length()<3){
			icNumber = icNumber.substring(icNumber.length() - 3,
					icNumber.length());
			// }
			content += "\nicNumber:" + icNumber;
			...
			Intent intent = new Intent(mContext,
					CardBalanceConfirmActivity.class);
		...
		public void onRequestSelectApplication(ArrayList<String> appList) {
			...
			appListView.setAdapter(new ArrayAdapter<String>(mContext,
		...
		public void onRequestTerminalTime() {
			...
			cashin_show_msg_text
					.setText(getString(R.string.request_terminal_time) + " "
							+ terminalTime);
		}
		...
		public void onError(Error errorState) {
			...
			if (errorState == Error.CMD_NOT_AVAILABLE) {
				...
				Toast.makeText(mContext, getString(R.string.invalid_format),
				...
				Toast.makeText(mContext, getString(R.string.input_invalid),
						Toast.LENGTH_LONG).show();
