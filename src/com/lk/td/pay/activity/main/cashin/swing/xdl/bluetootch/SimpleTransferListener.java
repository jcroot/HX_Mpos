...
import com.lk.td.pay.activity.main.cashin.swing.xdl.bluetootch.swiper.SwiperInterfaceImpl;
...
import com.whty.bluetooth.manage.util.IOUtils;
...
*/
			...
			if (MApplication.getInstance().getIc_pinInput_flag() == 0) {
				System.err.println("<br>请输入交易密码..." );
				doPinInput(swipResult);

			} else if(MApplication.getInstance().getIc_pinInput_flag() == 2){
				System.err.println("<br>请输入交易密码..." );
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					BigDecimal amt = MApplication.getInstance().getAmt();
					...
					try {
						...
						if (e instanceof ProcessTimeoutException) {
							...
							final CustomDialog.Builder builder = new CustomDialog.Builder(swingcardbyxdlbluetootchactivity);
							builder.setTitle("swipe failed:").setMessage("是否重新刷卡或插卡?");
							builder.setOkBtn("是", new OnClickListener() {
							...
							builder.setCancelBtn("否", new OnClickListener() {
	...
	private void reDoSwipeCard() {
		...
		try {
			...
			if (e instanceof ProcessTimeoutException) {
				...
				final CustomDialog.Builder builder = new CustomDialog.Builder(swingcardbyxdlbluetootchactivity);
				builder.setTitle("swipe failed:");
				builder.setMessage("是否重新刷卡或插卡?");
				builder.setOkBtn("是", new OnClickListener() {
