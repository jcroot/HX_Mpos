...
import com.lk.td.pay.activity.main.cashin.swing.xdl.AppExCode;
import com.lk.td.pay.activity.main.cashin.swing.xdl.bluetootch.Const.DataEncryptWKIndexConst;
import com.lk.td.pay.activity.main.cashin.swing.xdl.bluetootch.Const.MKIndexConst;
import com.lk.td.pay.activity.main.cashin.swing.xdl.bluetootch.Const.MacWKIndexConst;
import com.lk.td.pay.activity.main.cashin.swing.xdl.bluetootch.Const.PinWKIndexConst;
...
public class BuletootchControllerImpl implements BuletootchController {
	...
	public PinInputResult startPinInputWithoutKeyboard(String acctSymbol, byte[] pin) {
		...
		PinInput pinInput = (PinInput) deviceManager.getDevice().getStandardModule(ModuleType.COMMON_PININPUT);
		return pinInput.startPinInputWithoutKeyboard(new WorkingKey(PinWKIndexConst.DEFAULT_PIN_WK_INDEX) , PinManageType.MKSK, AccountInputType.USE_ACCT_HASH, acctSymbol, pin);
	}
	...
	public SwipResult swipCard(String msg, long timeout, TimeUnit timeUnit) {
		...
		try {
			...
			case COMMON_SWIPER:
				...
				PosData.getPosData().setErr(cardResultType.toString());
	...
	public SwipResult swipCardForPlain(String msg, long timeout, TimeUnit timeUnit) {
		...
		try {
			...
			case COMMON_SWIPER:
				...
				logger.info("========刷卡结果=1============" + cardResultType.toString());
				PosData.getPosData().setErr(cardResultType.toString());
				if (cardResultType == CardResultType.SWIPE_CARD_FAILED) {
					throw new DeviceRTException(AppExCode.GET_TRACKTEXT_FAILED, "swip failed!");
				}
	...
	public PinInputEvent startPininput(AccountInputType acctInputType, String acctHash, int inputMaxLen, boolean isEnterEnabled, String msg, long timeout) throws InterruptedException {
		...
		pinInput.startStandardPinInput(new WorkingKey(PinWKIndexConst.DEFAULT_PIN_WK_INDEX), PinManageType.MKSK, acctInputType, acctHash, inputMaxLen, new byte[] { 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F' }, isEnterEnabled, msg, (int) timeout, TimeUnit.MILLISECONDS, listener);
		try {
			listener.startWait();
		} catch (InterruptedException e) {
	...
	@Override
	public void startTransfer(final Context context, OpenCardType[] openCardType, String msg, BigDecimal amt, long timeout, TimeUnit timeunit, CardRule opencardrule, SimpleTransferListener transferListener) throws Exception {
		isConnected();
		CardReader cardReader = (CardReader) deviceManager.getDevice().getStandardModule(ModuleType.COMMON_CARDREADER);
		if (cardReader == null) {
			throw new DeviceRTException(AppExCode.GET_TRACKTEXT_FAILED, "not support read card!");
	...
	@Override
	public void OpenCardReader(Context context, OpenCardType[] openCardTypes, CardRule cardRule, final String msg, final long timeout, final TimeUnit timeunit, SimpleTransferListener transferListener) throws Exception {
		isConnected();
		...
		case COMMON_SWIPER: {
			...
			if (swipRslt.getRsltType() == SwipResultType.SUCCESS) {
//				((MApplication) ((SwingCardByXDLBluetootchActivity) context).getApplication()).setSwipResult(swipRslt);
				return;
			}
			throw new DeviceRTException(AppExCode.GET_TRACKTEXT_FAILED, "swip failed:" + swipRslt.getRsltType());
		}
		case COMMON_ICCARD: {
			((MApplication) ((SwingCardByXDLBluetootchActivity) context).getApplication()).setOpen_card_reader_flag(1);
